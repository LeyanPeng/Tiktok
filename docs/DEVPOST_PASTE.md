...
agent stops asking once the customer has explicitly declined to state a
preference three times running — judged by evidence rather than by a fixed
count, so the behaviour is correct whether a session carries three constraints,
four, or six.
...
probability that a customer holds a constraint of that type, times the entropy of
that attribute across the surviving candidates. An attribute every candidate
agrees on carries zero information and asking about it burns a turn. When the
agent's own top-5 stops moving for two consecutive turns after information has
arrived, it switches selection criteria at runtime — dropping the prior term and
choosing purely by pool divergence.

That first probability is itself derived rather than assumed. A sensitivity sweep
showed it carries real leverage and a badly asymmetric one: a reversed prior costs
0.099, while having no prior at all costs only 0.008. Being wrong about it is far
more expensive than being right. So instead of hardcoding the distribution measured
across the 200 public sessions — which is only correct if the private split matches
— we replay the simulator's own span-selection logic across all 50,000 products and
take the population distribution. A parity check runs on every execution and
confirms the derived ordering still reproduces the measured one exactly.
...
may disable network access**, and require each team to declare whether their
system needs it. Pillar I prescribes a pipeline ending in *LLM Semantic
Ranking* — which would need a declared offline fallback to survive that
condition.
...
### What a 1.000 Hit Rate does not mean

We declined the `"other"` shortcut because it exploits an artifact of the
simulator. Fairness requires admitting that our main mechanism rests on a related
one: the simulated customer's requirements are sliced verbatim out of the target
product's own metadata, so a phrase like *"Shaft measures approximately 8.37 from
arch"* is a spec-sheet fragment, not something a person would say. Our
highest-weighted signal matches exactly those fragments.

So a Hit Rate of 1.000 means this agent is good at *this simulator*, not that it
is good at conversational shopping. Against real customers the verbatim route
would contribute close to nothing, and performance would fall back to what
category pruning, IDF coverage and the popularity prior carry alone — a number we
have not measured and do not claim.

The distinction we would defend is narrower than "clean versus not": `"other"`
bypasses the customer entirely, while verbatim matching still requires listening,
tracking state across turns, and choosing what to ask. That is a difference of
degree. The parts we would expect to transfer are the architecture — bucket
pruning, additive scoring without conjunctive filtering, slot decay on override,
information-gain clarification — not the matcher that carries most of the score.
...
**Prior work referenced:** ProductAgent (Ye, Jiang, Wang, Li, Li, Zheng, Xie,
Huang — *Benchmarking Conversational Product Search Agent with Asking
Clarification Questions*, arXiv:2407.00942; ACL Anthology 2025.emnlp-industry.25) informed three design choices — grounding clarification
...
- **Demo video:** https://youtu.be/gyuh9YjiGA4
